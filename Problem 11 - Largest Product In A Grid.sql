...
--CREATE RESULT TEMP TABLE IF NEEDED

DROP TABLE IF EXISTS #result SELECT 0 AS result, 0 AS compare INTO #result
...
WHILE @i <= 17
BEGIN
PRINT 'CALCULATING HORIZONTAL'
	SET @sql = CONCAT(N'
		UPDATE #result SET compare = (SELECT MAX([', @i,'] * [', @i + 1,'] * [', @i + 2,'] * [', @i + 3,']) AS maxProduct
		FROM #grid)'
		)
	PRINT @sql
	EXEC sp_executesql @sql
	SET @log = (SELECT CONCAT('New Compare = ',compare,' Old Result = ',result) FROM #result) PRINT @log
	UPDATE #result SET result = compare WHERE compare > result
	SET @log = (SELECT CONCAT('Result: ',result,' Compare: ',compare,' @i: ',@i,' @j: ',@j) FROM #result) PRINT @log 
	SET @i = @i + 1
END
...
BEGIN
	...
	WHILE @j <= 17
	BEGIN
		PRINT 'CALCULATING VERTICAL'
		SET @sql = CONCAT(N'UPDATE #result 
			SET compare = (SELECT EXP(SUM(LOG([',@i,']))) product FROM #grid WHERE rowNum BETWEEN ',@j,' AND ',@j+3,' AND (SELECT MIN([',@i,']) FROM #grid WHERE rowNum BETWEEN ',@j,' AND ',@j+3,') > 0)'
			)
		SET @log = (SELECT CONCAT('Result: ',result,' Compare: ',compare,' @i: ',@i,' @j: ',@j) FROM #result)
		PRINT @log PRINT @sql
		EXEC sp_executesql @sql
		UPDATE #result SET result = compare WHERE compare > result
		SET @j = @j + 1
...
BEGIN
	...
	WHILE @j <= 17
	BEGIN
	PRINT 'CALCULATING DIAGONAL TOP LEFT TO BOTTOM RIGHT'
	SET @sql = CONCAT(N'
		UPDATE #result SET compare =  
			(SELECT [',@i+0,'] FROM #grid WHERE rownum = ',@j+0,') *
			(SELECT [',@i+1,'] FROM #grid WHERE rownum = ',@j+1,') *
			(SELECT [',@i+2,'] FROM #grid WHERE rownum = ',@j+2,') *
			(SELECT [',@i+3,'] FROM #grid WHERE rownum = ',@j+3,')'
		)
		PRINT @sql
		EXEC sp_executesql @sql
		SET @log = (SELECT CONCAT('New Compare = ',compare,' Old Result = ',result) FROM #result) PRINT @log
...
BEGIN
	...
	WHILE @j <= 17
	BEGIN
	PRINT 'CALCULATING DIAGONAL BOTTOM LEFT TO TOP RIGHT'
	SET @sql = CONCAT(N'
		UPDATE #result SET compare =  
			(SELECT [',@i+0,'] FROM #grid WHERE rownum = ',@j+3,') *
			(SELECT [',@i+1,'] FROM #grid WHERE rownum = ',@j+2,') *
			(SELECT [',@i+2,'] FROM #grid WHERE rownum = ',@j+1,') *
			(SELECT [',@i+3,'] FROM #grid WHERE rownum = ',@j+0,')'
		)
		PRINT @sql
		EXEC sp_executesql @sql
		SET @log = (SELECT CONCAT('New Compare = ',compare,' Old Result = ',result) FROM #result) PRINT @log
